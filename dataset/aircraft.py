...
def load_data(root, batch_size, num_workers):
    Aircraft.init(root)
    test_dataset = Aircraft(root, 'test', test_transform())
    train_dataset = Aircraft(root, 'train', train_transform())
    base_dataset = Aircraft(root, 'train', test_transform())
...
class Aircraft(Dataset):
    ...
    def __init__(self, root, mode, transform=None, loader=default_loader):
        ...
        if mode == 'train':
            ...
            self.data = Aircraft.TEST_DATA
            self.targets = Aircraft.TEST_TARGETS
        else:
    ...
    def init(root):
        ...
        for i in range(len(train_images)):
            images_train.append(
                [img_id_train[i], 'fgvc-aircraft-2013b/data/images/' + train_images[i], label_list_train[i]])

        images_train = pd.DataFrame(images_train, columns=['img_id', 'filepath', 'target'])
        k = len(train_images)
        label_list_test = []
        img_id_test = []
        # print(images_test['filepath'][334:344])
        ...
        for i in range(len(test_images)):
            images_test.append(
                [img_id_test[i], 'fgvc-aircraft-2013b/data/images/' + test_images[i], label_list_test[i]])
        ...
        # Split dataset
        Aircraft.TEST_DATA = test_data['filepath'].to_numpy()
        Aircraft.TEST_TARGETS = encode_onehot((test_data['target'] - 1).tolist(), 100)
