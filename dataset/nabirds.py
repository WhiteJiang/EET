...
def load_data(root, batch_size, num_workers):
    NABirds.init(root)
    test_dataset = NABirds(root, 'test', test_transform())
    train_dataset = NABirds(root, 'train', train_transform())
    base_dataset = NABirds(root, 'train', test_transform())
...
class NABirds(VisionDataset):
    ...
    def __init__(self, root, mode, transform=None, target_transform=None):
        ...
        if mode == 'train':
            ...
            self.data = NABirds.TEST_DATA
            self.targets = NABirds.TEST_TARGETS
        else:
    ...
    def init(root):
        ...
        NABirds.TEST_DATA = test_data['filepath'].to_numpy()
        NABirds.TEST_TARGETS = encode_onehot((test_data['target']).tolist(), class_num)
